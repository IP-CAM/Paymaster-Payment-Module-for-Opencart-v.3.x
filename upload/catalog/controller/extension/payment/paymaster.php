...
class ControllerExtensionPaymentPaymaster extends Controller {
    ...
    public function index(): string {
        $data['action'] = 'index.php?route=extension/payment/paymaster/confirm';
    ...
    public function confirm(): void {
        ...
        $base_url = ((empty($_SERVER['HTTPS']) || $_SERVER['HTTPS'] == 'off') ? 'http' : 'https') . "://$_SERVER[HTTP_HOST]/index.php?route=extension/payment/paymaster/";
        $request_data = array(
        ...
        if (isset($response)) {
            ...
            if ($response->status == 'Settled' &&
                ...
                $response->amount->currency == $order['currency_code']) {
                    ...
                    $this->load->language('extension/payment/paymaster');
                    $this->model_checkout_order->addOrderHistory($order['order_id'], (int)$this->config->get('payment_paymaster_done_status_id'), $this->language->get('text_paid_with_paymaster'));
                }
        }
        return $status;
    }
